...
const NewArrival = ({ products, loading,   }) => {
                ...
                <div className="flex items-center justify-center gap-10 mb-10">
                    {
                        newArrivalsCategories.map((item, index) => (
                            <button
                                className={`text-base font-poppins font-normal capitalize cursor-pointer ${item.category     ? 'px-6 py-2.5 bg-black rounded-sm text-white' : 'text-[#8a8a8a] '}`}
                                key={index}
                                onClick={() => {
                                ...
                                products?.map((product) => (
                                    <div key={product?.id} className="bg-white shadow-lg p-5 rounded-md ">
                                        <div className="w-full max-h-[244px] h-full mb-2.5">
                                            <img className="w-full h-full object-cover rounded-md" src={product?.title[0]?.url} alt={product?.title} />
                                        </div>
                                        <div>
